...
static void print(const VoxTransform &o) {
  ...
  for (int i = 0; i < o.numFrames; ++i) {
    printf(" ++ FRAME ++ \n");
    printf("    + Translation %d %d %d\n", o.initialFrame.translation[0],
           o.initialFrame.translation[1], o.initialFrame.translation[2]);
...
static void print(const VoxShape &o) {
  ...
  for (auto m = o.models.begin(); m != o.models.end(); ++m) {
    printf("[%d => %s .. %s], ", m->first, m->second.first.c_str(),
           m->second.second.c_str());
  }
...
bool isSupportedChunk(const std::string &chunkName) {
  return true;
  return (chunkName == "MAIN" || chunkName == "SIZE" || chunkName == "XYZI" ||
          chunkName == "RGBA" || chunkName == "nGRP" || chunkName == "nTRN" ||
          chunkName == "nSHP");
}
...
bool VoxReader::decodeChunk(const std::string &chunkName,
                            const uint8_t *content, unsigned int size) {
  if (chunkName == "SIZE") {
...
void VoxReader::readChunk(const uint8_t *bytes, unsigned int size) {
  ...
  if (!isSupportedChunk(chunkName)) {
    ...
    if (childChunkContentSize > 0) {
      ...
      memcpy(&childChunkContent, bytes + 12 + chunkContentSize,
             childChunkContentSize);
    ...
    if (remainingSize > 0) {
      readChunk(&bytes[12 + chunkContentSize + childChunkContentSize],
                remainingSize);
    }
  }
}
...
{
  if (bytes[0] != 'V' || bytes[1] != 'O' || bytes[2] != 'X' ||
      bytes[3] != ' ') {
